...
Vector3 getMaxCoordinates(std::vector<Vector3> vertexs)
{
	Vector3 maxVector = vertexs[0];
	for (int i = 1; i < vertexs.size() ; i++)
	{
		maxVector.x = max(maxVector.x, vertexs[i].x);
		maxVector.y = max(maxVector.y, vertexs[i].y);
		maxVector.z = max(maxVector.z, vertexs[i].z);
	}
	return maxVector;
}

Vector3 getMinCoordinates(std::vector<Vector3> vertexs)
{
	Vector3 minVector = vertexs[0];
	for (int i = 1; i < vertexs.size(); i++)
	{
		minVector.x = min(minVector.x, vertexs[i].x);
		minVector.y = min(minVector.y, vertexs[i].y);
		minVector.z = min(minVector.z, vertexs[i].z);
	}
	return minVector;
}
...
SDL_Joystick* openJoystick(int num_joystick) {
	...
	if (found <= num_joystick)
	{
		std::cout << "JoyStick " << num_joystick << " not available" << std::endl;
		return NULL;
	}
